...
export async function POST(request: Request, context: Context) {
  ...
  if (field(form, "closure_confirmed") !== "yes") return NextResponse.redirect(new URL(`/interno/projetos/${id}?status=closure_confirmation_required`, request.url), 303);
  ...
  if (!deliverySummary || !deliveryEvidenceReference || !resultSummary || !lessonsLearned || !feedbackStatuses.has(clientFeedbackStatus) || !nextSteps.has(nextStep)) return NextResponse.redirect(new URL(`/interno/projetos/${id}?status=closure_invalid`, request.url), 303);

  try {
    await prepareProjectClosure({ projectId: id, actorLabel: session.user, deliverySummary, deliveryEvidenceReference, resultSummary, lessonsLearned, clientFeedbackStatus, clientFeedbackNotes, financePendingNote, nextStep, reassessmentRequired });
