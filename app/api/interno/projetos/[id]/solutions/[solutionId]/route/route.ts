...
import { isSolutionCatalogSchemaPending, setProjectSolutionRoute } from "../../../../../../../../lib/blinko/solution-catalog-server";
...
export async function POST(request: Request, context: Context) {
  ...
  if (!uuidPattern.test(id) || !uuidPattern.test(solutionId) || !confirmed || !routePattern.test(route)) return NextResponse.redirect(new URL(`/interno/projetos/${id}/onboarding?status=solution_route_invalid`, request.url), 303);
  ...
  try {
    ...
    if (!isSolutionCatalogSchemaPending(error)) console.error("Blinko OS: falha ao confirmar rota de solução", error);
