...
import { getProjectWorkspace, isExecutionSchemaPending, setProjectOnboardingItem } from "../../../../../../lib/blinko/execution-server";
...
export async function POST(request: Request, context: Context) {
  ...
  if (!confirmed || !moduleCode || !requirements.has(requirement) || !statuses.has(onboardingStatus) || (dueAtLocal && !localDateTimePattern.test(dueAtLocal)) || (nextCheckAtLocal && !localDateTimePattern.test(nextCheckAtLocal))) return NextResponse.redirect(new URL(`/interno/projetos/${id}?status=onboarding_item_invalid`, request.url), 303);
  ...
  try {
    ...
    if (!workspace.schemaReady || !workspace.onboardingSchemaReady || workspace.project?.status !== "onboarding" || !itemExists) return NextResponse.redirect(new URL(`/interno/projetos/${id}?status=onboarding_item_blocked`, request.url), 303);

    await setProjectOnboardingItem({ projectId: id, moduleCode, requirement, status: onboardingStatus, evidence, responsibleLabel, dueAt: dueAtLocal ? new Date(`${dueAtLocal}:00-03:00`).toISOString() : null, blockingReason, blockingOwnerLabel, nextCheckAt: nextCheckAtLocal ? new Date(`${nextCheckAtLocal}:00-03:00`).toISOString() : null, notes, actorLabel: session.user });
    return NextResponse.redirect(new URL(`/interno/projetos/${id}?status=onboarding_item_saved`, request.url), 303);
  } catch (error) {
    if (isExecutionSchemaPending(error)) return NextResponse.redirect(new URL(`/interno/projetos/${id}?status=onboarding_schema_pending`, request.url), 303);
