...
import { finalizeDiagnosticStrategy, getDiagnosticStrategyContext, isDiagnosticStrategySchemaPending } from "../../../../../../../lib/blinko/diagnostic-strategy-server";
...
export async function POST(request: Request, context: Context) {
  ...
  if (String(form.get("strategy_confirmed") ?? "") !== "yes") return NextResponse.redirect(new URL(`/interno/diagnosticos/${id}?status=strategy_finalize_confirmation_required`, request.url), 303);
  ...
  try {
    ...
    if (diagnosticStatus !== "review") return NextResponse.redirect(new URL(`/interno/diagnosticos/${id}?status=strategy_finalize_blocked`, request.url), 303);
    ...
    const hasCompleteChain = strategyContext.interventions.some((item) => item.status === "selected" && confirmedProblems.has(item.problem_id) && confirmedCauses.has(item.cause_id) && selectedPriorities.has(item.priority_id));
    if (!hasCompleteChain) return NextResponse.redirect(new URL(`/interno/diagnosticos/${id}?status=strategy_finalize_blocked`, request.url), 303);
