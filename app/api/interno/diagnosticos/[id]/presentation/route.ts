...
import { getProposalContext, isProposalSchemaPending, recordDiagnosticPresentation } from "../../../../../../lib/blinko/proposal-server";
...
export async function POST(request: Request, context: Context) {
  ...
  if (!confirmed || !localDateTimePattern.test(presentedAtLocal)) return NextResponse.redirect(new URL(`/interno/diagnosticos/${id}?status=presentation_invalid`, request.url), 303);

  try {
    const proposalContext = await getProposalContext(id);
    if (!proposalContext.schemaReady) return NextResponse.redirect(new URL(`/interno/diagnosticos/${id}?status=diagnostic_schema_pending`, request.url), 303);
    if (proposalContext.diagnostic?.status !== "ready_for_presentation") return NextResponse.redirect(new URL(`/interno/diagnosticos/${id}?status=presentation_blocked`, request.url), 303);
    ...
    if (isProposalSchemaPending(error)) return NextResponse.redirect(new URL(`/interno/diagnosticos/${id}?status=diagnostic_schema_pending`, request.url), 303);
