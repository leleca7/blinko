...
import {
  getStructuredDiagnosticWorkspace,
  isStructuredDiagnosticSchemaPending,
  saveStructuredDiagnosticItem,
} from "../../../../../../lib/blinko/diagnostic-structured-server";
...
export async function POST(request: Request, context: Context) {
  ...
  if (!uuidPattern.test(collectionVersionId) || !itemPattern.test(itemCode) || !["nv","na","0","1","2","3","4"].includes(rawState)) {
    return NextResponse.redirect(new URL(`/interno/diagnosticos/${id}?status=structured_item_invalid`, request.url), 303);
  }

  const responseState = rawState === "nv" ? "nv" : rawState === "na" ? "na" : "score";
  const score = responseState === "score" ? Number(rawState) : null;
  const relatedPillars = text(form.get("related_pillars"), 160)
    .split(/[,;\s]+/)
    .map((item) => item.toUpperCase())
    .filter((item, index, array) => pillarPattern.test(item) && array.indexOf(item) === index)
    .slice(0, 10);
  ...
  try {
    ...
    if (!workspace.diagnostic || !structured.schemaReady || structured.collectionVersionId !== collectionVersionId) {
      return NextResponse.redirect(new URL(`/interno/diagnosticos/${id}?status=structured_item_invalid`, request.url), 303);
    }
    const status = typeof workspace.diagnostic.status === "string" ? workspace.diagnostic.status : "";
    if (!["collection","analysis"].includes(status)) {
      return NextResponse.redirect(new URL(`/interno/diagnosticos/${id}?status=structured_item_locked`, request.url), 303);
    }
    ...
    if (isStructuredDiagnosticSchemaPending(error)) {
      return NextResponse.redirect(new URL(`/interno/diagnosticos/${id}?status=structured_schema_pending`, request.url), 303);
    }
