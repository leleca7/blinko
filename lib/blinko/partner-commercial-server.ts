...
export function isPartnerCommercialSchemaPending(error: unknown) {
...
export type ProposalPartnerContext = {
  schemaReady: boolean;
...
export async function getProposalPartnerContext(proposalId: string): Promise<ProposalPartnerContext> {
  ...
  try {
        ...
        'interventions', coalesce((
          select jsonb_agg(
            jsonb_build_object(
              'id',i.id,
              'title',i.title,
              'solution_code',b.official_code,
              'solution_name',b.name,
              'selected_route',i.selected_execution_route,
              'allowed_routes',to_jsonb(b.execution_routes),
              ...
              'commitment',(
                select jsonb_build_object(
                  ...
                  'approval_scope',pc.approval_scope,
                  'execution_route',pc.execution_route,
                  'blinko_role',pc.blinko_role,
                  'quote_reference',pc.quote_reference,
                  'quoted_at',pc.quoted_at,
                  'quote_valid_until',pc.quote_valid_until,
                  'quoted_cost_to_blinko',pc.quoted_cost_to_blinko,
                  'blinko_payment_obligation',pc.blinko_payment_obligation,
                  'payment_terms_snapshot',pc.payment_terms_snapshot,
                  'condition_evidence',pc.condition_evidence,
                  'validation_evidence',pc.validation_evidence,
                  'partner_id',pt.id,
                  'partner_code',pt.official_code,
                  'partner_name',pt.trade_name,
                  'partner_eligibility',public.partner_operational_eligibility(pt.id),
                  'financial_rule_id',pc.financial_rule_id,
                  'financial_rule_auto_calculable',case when pc.financial_rule_id is null then false else public.partner_financial_rule_auto_calculable(pc.financial_rule_id) end,
                  'ready',public.proposal_partner_commitment_is_ready(pc.id)
                )
                from public.proposal_partner_commitments pc
                join public.partners pt on pt.id=pc.partner_id
                where pc.proposal_version_id=p.current_version_id and pc.intervention_id=i.id and pc.is_current
                limit 1
              ),
              'partner_options',coalesce((
                select jsonb_agg(jsonb_build_object(
                  ...
                  'registration_status',pt.registration_status,
                  'eligibility',public.partner_operational_eligibility(pt.id),
                  'capability_status',cap.status,
                  'financial_rule_id',fr.id,
                  'financial_rule_model',fr.remuneration_model,
                  'financial_rule_status',fr.formalization_status,
                  'financial_rule_percentage',fr.percentage_value,
                  'financial_rule_base_status',fr.calculation_base_status,
                  'financial_rule_auto_calculable',case when fr.id is null then false else public.partner_financial_rule_auto_calculable(fr.id) end
                ...
                left join lateral (
                  ...
                  order by case when r.blueprint_id=b.id then 0 else 1 end,r.version_number desc
                  limit 1
                ) fr on true
    ...
    return {
      schemaReady: true,
      requirementsReady: result?.requirements_ready === true,
...
export async function refreshProposalPartnerQuote(input: {
  commitmentId: string; quoteReference: string; quotedAt: string; quoteValidUntil?: string | null; evidence: string; actorLabel: string;
}) {
  const sql = getSql();
  const quoteValidUntil = input.quoteValidUntil || null;
  const rows = await sql`
    select public.refresh_proposal_partner_quote_validity(
      ${input.commitmentId}::uuid,${input.quoteReference},${input.quotedAt}::timestamptz,${quoteValidUntil}::timestamptz,${input.evidence},${input.actorLabel}
    ) as result
  `;
  return rows[0]?.result as string;
}
  ...
  try {
    const rows = await sql`
      select coalesce(jsonb_agg(jsonb_build_object(
        ...
        'committed_cost_to_blinko',pa.committed_cost_to_blinko,'blinko_payment_obligation',pa.blinko_payment_obligation,
        'payment_terms_snapshot',pa.payment_terms_snapshot,'partner_id',pt.id,'partner_code',pt.official_code,
        'partner_name',pt.trade_name,'partner_eligibility',public.partner_operational_eligibility(pt.id),
        'solution_code',ps.solution_code,'project_solution_id',ps.id,'financial_rule_id',pa.financial_rule_id,
        'financial_rule_model',fr.remuneration_model,'financial_rule_status',fr.formalization_status,
        'financial_rule_auto_calculable',case when fr.id is null then false else public.partner_financial_rule_auto_calculable(fr.id) end,
        'cost',case when pc.id is null then null else jsonb_build_object('id',pc.id,'amount',pc.amount,'status',pc.status,'payment_reference',pc.payment_reference) end
